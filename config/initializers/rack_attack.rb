...
class Rack::Attack
  ...
  throttle('req/ip', limit: 1, period: 1.minutes) do |req|
  ...
  blocklist('fail2ban') do |req|
    Fail2Ban.filter("pentesters-#{req.ip}", maxretry: 0, findtime: 1.minutes, bantime: 1.hour) do
      configuration.throttled?(req)
    end
  end
end

ActiveSupport::Notifications.subscribe(/(throttle|blocklist).rack_attack/) do |name, start, finish, request_id, payload|
  req = payload[:request]
