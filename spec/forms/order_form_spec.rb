  ...
  describe '#save' do
    ...
    context 'when success' do
      let(:order) do
        {
          buyer_id: buyer.id,
          price: (stock1.price * 2 + stock2.price * 1)
        }.with_indifferent_access
      end
      let(:order_item1) do
        {
          game_id: stock1.game_id,
    ...
    context 'when checkout fail' do
      before do
        allow_any_instance_of(Cart).to receive(:checkout!)
          .and_raise(Cart::CheckoutError)
      end
      ...
      context 'Cart::CheckoutError will be rescue and add in errors' do
        ...
        it 'will add message in error' do
          subject
          expect(form.errors).to have_received(:add)
            .with(:cart, Cart::CheckoutError.new.message)
        end
      end
    ...
    context 'when order create fail' do
      before do
        allow_any_instance_of(Order).to receive(:save!)
      ...
      context 'ActiveRecord::RecordInvalid will be rescue and add in errors' do
        ...
        it 'will add message in error' do
          subject
          expect(form.errors).to have_received(:add)
            .with(:order, ActiveRecord::RecordInvalid.new.message)
        end
      end
    end
  end
